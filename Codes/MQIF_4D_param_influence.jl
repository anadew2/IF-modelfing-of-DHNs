...
# ╔═╡ 0b408815-0368-4ba1-9570-f63b7d4b262b
p_level = change_I_(130,change_vs0_(-38.4,change_vus0_(30,change_gs_(0.9,change_guus_(0.0005,change_gus_(0.01,p))))))

# ╔═╡ 55ea495a-3ac3-44f0-8429-af826074ec89
md"""Corresponding bifurcation current is $(round(find_global_bif(p_level)*100)/100)

$(find_local_bif(p_level))

"""

# ╔═╡ 887c048a-9b26-49f2-8201-665cffe55729
begin
	u0_l=[-40,-40,-40,-40]
	tspan_l = (0.0,7500.0)
	prob_l = ODEProblem(MQIF_4D!,u0_l,tspan_l,p_level,callback=cb)
	sol_l = solve(prob_l,dense=false,reltol=1e-6,abstol=1e-6)
	md"""ODE solution"""
end

# ╔═╡ 88b59847-f01b-47e1-a6ac-beadf44fbfa8
begin
	plot(sol_l.t,sol_l[1,:],linecolor="blue",label="V(t)")
	plot!(sol_l.t,sol_l[2,:],linecolor="orange",label="Vs(t)")
	plot!(sol_l.t,sol_l[3,:],linecolor="pink",label="Vus(t)")
	plot!(sol_l.t,sol_l[4,:],linecolor="black",label="Vuus(t)")
	plot!(sol_l.t,p_level[3]ones(size(sol_l.t)),linecolor="red",label="Vs0")
	plot!(sol_l.t,p_level[4]ones(size(sol_l.t)),linecolor="magenta",label="Vus0")
	plot!(sol_l.t,p_level[5]ones(size(sol_l.t)),linecolor="green",label="Vuus0")
...
# ╔═╡ 88ad9a18-80a5-4735-8229-b9f166f04711
begin
	Iuus_l = compute_Iuus(sol_l[4,:],p_level)
	Ius_l = compute_Ius(sol_l[3,:],p_level)
	It_l = p_level[1].+Iuus_l.+Ius_l
	
	plot(sol_l.t,It_l)
	plot!(sol_l.t,find_global_bif(p_level).*ones(size(sol_l.t)))
	plot!(sol_l.t,sol_l[3,:])
end
...
begin
	plt_dic,plt_c_max = dic(p_level,true)
	plt_c_max
end
...
# ╔═╡ 6c93a71c-7222-4071-b105-4500dad10e52
p_level2 = change_I_(30,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.0001,change_gus_(0.02,p))))))

# ╔═╡ 21fd5b56-f512-452b-a5f8-a6a8a8646d40
md"""Corresponding bifurcation current is $(round(find_global_bif(p_level2)*100)/100)

$(find_local_bif(p_level2))

"""

# ╔═╡ ec3d5746-2996-45a1-bbd8-abafed577bf3
begin
	u0_l2=[-50,-50,-50,-50]
	tspan_l2 = (0.0,7500.0)
	prob_l2 = ODEProblem(MQIF_4D!,u0_l2,tspan_l2,p_level2,callback=cb)
	sol_l2 = solve(prob_l2,DP5(),reltol=1e-12,abstol=1e-12)
	md"""ODE solution"""
end

# ╔═╡ 61296853-56c5-4935-8c68-97b3a78c4957
begin
	plot(sol_l2.t,sol_l2[1,:],linecolor="blue",label="V(t)")
	plot!(sol_l2.t,sol_l2[2,:],linecolor="orange",label="Vs(t)")
	plot!(sol_l2.t,sol_l2[3,:],linecolor="pink",label="Vus(t)")
	plot!(sol_l2.t,sol_l2[4,:],linecolor="black",label="Vuus(t)")
	plot!(sol_l2.t,p_level2[3]ones(size(sol_l2.t)),linecolor="red",label="Vs0")
	plot!(sol_l2.t,p_level2[4]ones(size(sol_l2.t)),linecolor="magenta",label="Vus0")
	plot!(sol_l2.t,p_level2[5]ones(size(sol_l2.t)),linecolor="green",label="Vuus0",size=(300,300))
	#scatter!(sol.t[ind_I_0],v[ind_I_0])
	#scatter!([sol.t[ind1],sol.t[ind2]],[v[ind1],v[ind2]])
	yaxis!("Voltage")
	xaxis!("Time [ms]")
...
# ╔═╡ 7239141d-ba76-4633-872f-165f1b8ea453
begin
	Iuus_l2 = compute_Iuus(sol_l2[4,:],p_level2)
	Ius_l2 = compute_Ius(sol_l2[3,:],p_level2)
	It_l2 = p_level2[1].+Iuus_l2.+Ius_l2
	
	plot(sol_l2.t,It_l2)
	plot!(sol_l2.t,find_local_bif(p_level2).*ones(size(sol_l2.t)),size=(300,200))
	xaxis!((0.0,200))
end

# ╔═╡ f7086e60-9e1b-4537-b85f-fb600363f8cb
p_level3 = change_I_(30,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.00001,change_gus_(0.001,p))))))

# ╔═╡ 045da4d0-010b-4eee-bbfb-ba2adcc2dab4
begin
	u0_l3=[-60,-60,-60,-60]
	tspan_l3 = (0.0,7500.0)
	prob_l3 = ODEProblem(MQIF_4D!,u0_l3,tspan_l3,p_level3,callback=cb)
	sol_l3 = solve(prob_l3,DP5(),reltol=1e-12,abstol=1e-12)
	
	plot(sol_l3.t,sol_l3[1,:],linecolor="blue",label="V(t)")
	plot!(sol_l3.t,sol_l3[2,:],linecolor="orange",label="Vs(t)")
	plot!(sol_l3.t,sol_l3[3,:],linecolor="pink",label="Vus(t)")
	plot!(sol_l3.t,sol_l3[4,:],linecolor="black",label="Vuus(t)",size=(300,200))
...
# ╔═╡ 3a229e05-6390-4fad-922d-1d14338bb0c1
md""" Categories for classification

	TONIC
						f~cst during pulse,
						stable before/after pulse,
						limit cycle for Ih cst 
						threshold for variation of frequency 
.

	ACCELERATING
						f increases during pulse,
						stable before/after pulse,
						limit cycle for Ih cst 
							(=no bursting)
.
	...
	PLATEAU POTENTIALS 	
						f increases during pulse,			f increases during pulse,
						stable before pulse,				stable before pulse,
						f decreases after pulse,			f decreases after pulse,
						limit cycle for Ih cst 				Burst for Ih cst 	
							(=no bursting)					burst period >3*pulse span
.
...
.
	...
	STABLE
						Stable state pulse end, 
						stable state for Ih cst 
					
.

	UNSTABLE 
						cycle with f~cst after pulse
						no burst for Ih 
.

	DECELERATING  											+plateau
						f decreases during pulse,			f decreases during puls	
						stable before/after pulse,			stable before pulse,
						limit cycle for Ih cst 				f decreases after pulse,
							(=no bursting)					limit cycle for Ih cst 	
																(=no bursting)
.
...
function freq_during_pulse(t,tspan_pulse,f_ins,t_ins)
	...
	if length(t_ins)>1
		...
		if abs(maximum(f_ins)-minimum([f_ins[ind_spike_pulse_0],f_ins[ind_spike_pulse_f]])) <=5 
			#constant 
			f_evol = 0
		else
...
function check_if_plateau(t,sim,tspan,tspan_pulse)
	ind_tmin = findfirst(x -> x >=maximum(tspan_pulse),t)
...
function plot_I_effect(p_root,Iapp)
	...
	##Plot I;It
	plot_I_It = plot(Iapp,It[1,:],linecolor=RGB(0.9,0.2,0.4),linewidth=2)
				plot!(Iapp,It[2,:],linecolor=RGB(1,0.4,0.4),linewidth=2,linestyle=:dot)
	...
	return plot_I,conv_4D
end

# ╔═╡ cbe7686f-4d76-4bd0-8983-c0a5633c22d6
begin
	Iapp=collect(range(10.5,stop=30,step=0.01))
	plot_I,conv4D=plot_I_effect(p,Iapp)
end
...
# ╔═╡ e85889bf-4e68-437c-aab8-a49412da99b6
begin
	conv3 = classify_pattern(0.0,10000.0,250.0,p_level3,0.5,10.0)
	translate_classification(conv3)
end
...
# ╔═╡ 4e2b20e0-44d4-4b19-aa2a-632d4659b4fe
begin
	Ibif_l = find_global_bif(p_level)
	subplot_simu_step_(0.0,10000.0,700.0,p_level,Ibif_l-30.0,Ibif_l+30.0,true,false,true)	
	plot!(size=(800,500))
	#savefig("MQIF_4D-simustep.pdf")
	#yaxis!((-50,50))
end

# ╔═╡ fb11132e-e886-4f2c-ba02-df1bf78c68a9
begin
	Ibif_l2 = find_global_bif(p_level2)
	subplot_simu_step_(0.0,10000.0,250.0,p_level2,5.0,Ibif_l2+5.0,true,false,true)	
	plot!(size=(800,500))
	#savefig("MQIF_4D-simustep.pdf")
	#yaxis!((-50,50))
end

# ╔═╡ 52059c99-ca9f-4539-b31e-217675c13351
begin
	p_plateau_pot_no_burst = change_I_(30,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.0001,change_gus_(0.02,p))))))
#change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.7,change_guus_(0.0001,change_gus_(0.008,p))))))
	
	p_tonic = change_I_(20.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.00003,change_gus_(0.0006,p)))))) #f is curved but max min variation of 4Hz and mean f around 205Hz
	
	p_decel_reg = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.75,change_guus_(0.0005,change_gus_(0.0000001,p))))))
	
	p_decel_plateau = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.82,change_guus_(0.0005,change_gus_(0.0000001,p))))))
	
	p_stable = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.99,change_guus_(0.0005,change_gus_(0.0000001,p))))))
	
	p_unstable_full = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.0001,change_gus_(0.0000001,p))))))
	
	p_unstable_after = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.0001,change_gus_(0.005,p))))))
	
	p_plateau_pot_nb_too_long_pulse = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.0001,change_gus_(0.01,p)))))) #f increases and decreases during the pulse ; Final f at end of pulse higher that initial f start of pulse 
	
	p_accelerating = change_I_(Ibif_l2+5.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.7,change_guus_(0.0005,change_gus_(0.005,p)))))) #f is curved (increases then decrases 
	
	dur_pulse = 250.0
	Il = 5.0
	Ibif_pp = find_global_bif(p_plateau_pot_no_burst)
	Ih = Ibif_pp+5.0
	md"""Parameters used to simulate various responses to a pulse of 250ms"""
end
...
p_level4 = p_plateau_pot_no_burst_sh#change_I_(20.0,change_vs0_(-38.4,change_vus0_(-10,change_gs_(0.5,change_guus_(0.1,change_gus_(0.000001,p))))))

# ╔═╡ 1fa67863-e6c0-48a3-a937-a8c42cd49c29
plt_bif,b_gs,gs = tune_bif(p_level4)
...
begin
	conv = classify_pattern(0.0,10000.0,100.0,p_level4,0.5,20)
...
begin
	...
	full_4 = subplot_simu_step_full(0.0,20000.0,201.0,p_level4,1.5,5.2,false,false,true)
	plot(title_4,full_4,
...
# ╔═╡ 9c352751-b62d-4032-a03f-2a799eca57d4
begin
	sim = zeros(4,length(sol_l4.t))
	sim[1,:].= sol_l4[1,:]
	sim[2,:].= sol_l4[2,:]
	sim[3,:].= sol_l4[3,:]
	sim[4,:].= sol_l4[4,:]
	full_frequency(sol_l4.t,sim,tspan_l4,(0.0,0.0))
end
...
# ╔═╡ 536d051a-0b29-4472-9255-9231430de185
begin
	plt = subplot_simu_step_(0.0,10000.0,250.0,p_level2,5.0,Ibif_l2+5.0,false,false,true)	
	#xaxis!((5000,5100))
end

# ╔═╡ 98ed5d88-9a79-4747-b622-fd3fbcb0df66
begin
	Ibif_l3 = find_global_bif(p_level3)
	subplot_simu_step_(0.0,10000.0,250.0,p_level3,0.5,10.0,true,false,true)	
end

# ╔═╡ e72929a6-3ff7-47d3-a2c7-0327feaacb73
subplot_simu_step_(0.0,10000.0,250.0,p_level3,0.5,10.0,false,false,true)	

# ╔═╡ 5df64bf0-fde6-4a8d-ab04-fbc3f4db77ff
begin
	Ibif_l4 = find_global_bif(p_level4)
	t_4 = subplot_simu_step_(0.0,10000.0,100.0,p_level4,0.5,20,true,false,true)	
end

# ╔═╡ e249b44d-f6e6-4e7b-9455-d76192da6cb6
begin
	f_4 = subplot_simu_step_(0.0,10000.0,100.0,p_level4,Ibif_l4-5.0,Ibif_l4+50.0,false,false,true)
end
